...
// for all owl people items, including clones - give a click event to open bios
$('#owl-people').on('click', '.item', function (event) {
  let clickedItemId = event.target.id;
  document.querySelector(`.${clickedItemId}`).classList.add('active');
...
if ($(window).width() > 768) {
  document.addEventListener('scroll', function () {
    let currScrollPos2 =
      ...
      0;
    if (currScrollPos2 > 1) {
      document.querySelector('.hero-text').style.opacity =
        -currScrollPos2 / 350 + 1.1;
    }
  });
  ...
  background_image_parallax($('.hero-text'), 0.4);
}
